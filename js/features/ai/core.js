...
function getOrCreatePort() {
    ...
    try {
        sharedPort = browserAPI.runtime.connect({ name: "rep-panel" });
        ...
        sharedPort.onDisconnect.addListener(() => {
            console.warn('Port disconnected, clearing shared port. Error:', browserAPI.runtime.lastError?.message);
        ...
        try {
            ...
            if (browserAPI.runtime.lastError) {
                ...
                if (!isResolved) {
                    isResolved = true;
                    portListeners.delete(requestId);
                    reject(new Error('Port error after sending: ' + browserAPI.runtime.lastError.message));
                }
            }
        } catch (e) {
            clearTimeout(timeout);
